...
def extract_obs_sequence(obs: Observation,
                cameras,
                t: int = 0,
                prev_action=None,
                channels_last: bool = False,
    ...
    # binary variable indicating if collisions are allowed or not while planning paths to reach poses
    obs_ignore_collisions = np.array([obs.ignore_collisions], dtype=np.float32)
    obs_dict['ignore_collisions'] = obs_ignore_collisions
    for (k, v) in [(k, v) for k, v in obs_dict.items() if 'point_cloud' in k]:
        obs_dict[k] = v.astype(np.float32)
    ...
    return obs_dict, obs_ignore_collisions
