...
class PointsRendererWithDepth(nn.Module):
    ...
    def forward(self, point_clouds, with_depth=False, **kwargs) -> torch.Tensor:
        ...
        images = images.permute(0, 2, 3, 1)   # torch.Size([5, 220, 220, 6])

        if with_depth:
            images = torch.cat((images, depth.unsqueeze(-1)), dim=-1)   # torch.Size([5, 220, 220, 7])
...
class BoxRenderer:  
    ...
    def __call__(self, pc, feat, fix_cam=True, dyn_cam_info=None):
        ...
        :param feat: torch.Tensor (num_point, num_feat)    #  num_feat =  6  add_corr
        ...
        if fix_cam:
            ...
            p3d_pc = p3d_pc.extend(self.num_fix_cam)   # copy pc num_cam times
