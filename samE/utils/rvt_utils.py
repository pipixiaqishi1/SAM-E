...
from samE.models.peract_official import PreprocessAgent2
...
def get_pc_img_feat(obs, pcd, bounds=None):
    ...
    # (bs, num_points, 3)
    pc = torch.cat([p.permute(0, 2, 3, 1).reshape(bs, -1, 3) for p in pcd], 1)   # cat (bs,128*128,3) * camera_nums  ->   (bs,128*128*camera_nums,3)
    _img_feat = [o[0] for o in obs]   # list of rgb [(bs,3,128,128) each camera ]
    img_dim = _img_feat[0].shape[1]
    # (bs, num_points, 3)
    img_feat = torch.cat(
        [p.permute(0, 2, 3, 1).reshape(bs, -1, img_dim) for p in _img_feat], 1  # cat (bs,128*128,3) * camera_nums  ->   (bs,128*128*camera_nums,3)
    )
...
def load_agent_model(agent_path, agent=None, only_epoch=False):
    if isinstance(agent, PreprocessAgent2):
        assert not only_epoch
        agent._pose_agent.load_weights(agent_path)
        return 0
