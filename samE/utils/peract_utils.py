...
from samE.models.peract_official import create_agent_our
...
def _preprocess_inputs_ck(act_idx,replay_sample, cameras):
    obs, pcds = [], []
    for n in cameras:
        rgb = stack_on_channel(replay_sample["%s_rgb_ck" % n][:,act_idx:act_idx+1])
        pcd = stack_on_channel(replay_sample["%s_point_cloud_ck" % n][:,act_idx:act_idx+1])
