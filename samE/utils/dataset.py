...
from samE.utils.peract_utils import LOW_DIM_SIZE, IMAGE_SIZE, CAMERAS
from samE.libs.peract.helpers.demo_loading_utils import keypoint_discovery
from samE.libs.peract.helpers.utils import extract_obs, extract_obs_sequence
...
def create_replay(
    ...
    if if_chunk:
        ...
        ReplayElement("rot_grip_action_indicies_ck", (5,rot_and_grip_indicies_size,), int),
        ReplayElement("gripper_pose_ck", (5,gripper_pose_size,), np.float32),
        ReplayElement("ignore_collisions_ck", (5,ignore_collisions_size,), np.float32),
        ])
        for cname in cameras:
            extra_replay_elements.append(
                ObservationElement(
                    "%s_rgb_ck" % cname,
                    (   5,
                        3,
...
def create_replay_sequence(
    ...
    extra_replay_elements = [
            ReplayElement("demo", (), bool),
            ReplayElement("obs_frame", (), int),
            ReplayElement("episode_idx", (), int),
            ReplayElement("keypoint_list", (25,), list),
            ReplayElement("keypoint_frame_list", (25,), list),
            ReplayElement("horizon", (), int),
            ReplayElement("obs_ignore_collisions", (ignore_collisions_size,), np.float32),
            ReplayElement("gripper_pose_list", (25,gripper_pose_size,), np.float32),
            ReplayElement("rot_grip_action_indicies_list", (25,rot_and_grip_indicies_size,), np.int32),
            ReplayElement("ignore_collisions_list", (25,), np.float32),
    ]

    replay_buffer = (
        UniformReplayBuffer_sequence(  # all tuples in the buffer have equal sample weighting
            disk_saving=disk_saving,
            batch_size=batch_size,
            timesteps=timesteps,
            replay_capacity=int(replay_size),
    ...
    if disk_exist:
        ...
        for d_idx in range(start_idx, start_idx + num_demos):
            ...
            for i in range(len(demo) - 1):
                ...
                if next_keypoint_idx == len(episode_keypoints):
                    break
                _add_keypoints_to_replay_sequence(
                    replay,
                    task,
                    task_replay_storage_folder,
                    d_idx,
        ...
        # for fill replay sequence, save replay_info.npy  when recover_from_disk.
        print("Replay filled with demos.")


# add sequence data points to a replay
def _add_keypoints_to_replay_sequence(
    replay: ReplayBuffer,
    task: str,
    task_replay_storage_folder: str,
    episode_idx: int,
    ...
    obs_dict, obs_ignore_collisions = extract_obs_sequence(     
            obs,
            CAMERAS,
            t=0,     # t for calculate time, represent t_th keypoint BUG
            prev_action=prev_action,
            episode_length=25,)
    tokens = clip.tokenize([description]).numpy()
    token_tensor = torch.from_numpy(tokens).to(device)
    with torch.no_grad():
        lang_feats, lang_embs = _clip_encode_text(clip_model, token_tensor)
    obs_dict["lang_goal_embs"] = lang_embs[0].float().detach().cpu().numpy()
    key_point_list = list(range(next_keypoint_idx, len(episode_keypoints)))
    horizon = len(key_point_list)
    
    max_horizon = 25
    if horizon > max_horizon:
        key_point_list = key_point_list[:max_horizon]
        
    gripper_pose_list = []
    rot_grip_action_indicies_list = []
    ignore_collisions_list = []
    for i, k in enumerate(key_point_list): 
        keypoint = episode_keypoints[k]
        obs_tp1 = demo[keypoint]    # keypoint frame
        obs_tm1 = demo[max(0, keypoint - 1)]   # frame before keypoint
        (
        ...
        gripper_pose_list.append(obs_tp1.gripper_pose)
        rot_grip_action_indicies_list.append(rot_grip_indicies)
        ignore_collisions_list.append(ignore_collisions)
       
    terminal = True if horizon == 1 else False
    reward = float(terminal) * 1.0 if terminal else 0
    keypoint_frame_list = [episode_keypoints[k] for k in key_point_list] 
    
    if horizon < max_horizon:
        for i in range(max_horizon-horizon):
            key_point_list.append(key_point_list[-1])
            keypoint_frame_list.append(keypoint_frame_list[-1])
            gripper_pose_list.append(gripper_pose_list[-1])
            rot_grip_action_indicies_list.append(rot_grip_action_indicies_list[-1])
            ignore_collisions_list.append(ignore_collisions_list[-1])
            
    info = {
        "demo": True,
        "obs_frame": sample_frame,
        "episode_idx": episode_idx,
        "keypoint_list": key_point_list,
        "keypoint_frame_list": keypoint_frame_list,
        'horizon': horizon,
        'lang_goal': np.array([description], dtype=object),
        'obs_ignore_collisions': obs_ignore_collisions,
        
    }
    action_info = {
        "gripper_pose_list": gripper_pose_list,  # 3+4
        "rot_grip_action_indicies_list": rot_grip_action_indicies_list,   # rot + open: 3+1
        "ignore_collisions_list": ignore_collisions_list,
    }
